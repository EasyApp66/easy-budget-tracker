...
export default function Budget() {
  ...
  const sortedExpenses = activeMonth 
    ? [...activeMonth.expenses].sort((a, b) => {
        if (a.pinned && !b.pinned) return -1;
        if (!a.pinned && b.pinned) return 1;
        return 0;
      })
  ...
  return (
    <div className="min-h-screen bg-background pb-32 safe-top">
      ...
      <EditModal
        ...
        currentValue={
          editTarget === 'month'
            ? months.find(m => m.id === selectedMonthId)?.name || ''
            : activeMonth?.expenses.find(e => e.id === selectedExpenseId)?.name || ''
        }
        type="text"
      />
